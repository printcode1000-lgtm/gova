...
async function main(): Promise<void> {
  ...
  const sliceName = hostProfile().sliceName;
  const sliceSource = path.join(backupRoot, "systemd", sliceName);
  if (existsSync(sliceSource)) {
    step(`install systemd user slice ${sliceName}`, () => {
      mkdirSync(SYSTEMD_USER_DIR, { recursive: true });
      writeFileSync(path.join(SYSTEMD_USER_DIR, sliceName), readFileSync(sliceSource, "utf8"), { mode: 0o644 });
    });
  }
