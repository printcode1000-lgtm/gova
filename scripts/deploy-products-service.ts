...
async function main(): Promise<void> {
  ...
  const runId = process.env.ASOL_DEPLOYMENT_RUN_ID?.trim() || `products-${Date.now()}`;
  const revision = process.env.ASOL_DEPLOYMENT_REVISION?.trim() || 'standalone';
  const comment = process.env.ASOL_DEPLOYMENT_COMMENT?.trim() || `products production deploy ${new Date().toISOString()}`;
  runVercel(['deploy', '--prod', '--yes', ...vercelDeploymentMetadata({ target: 'products', comment, runId, revision })], projectId, teamId);
  const report = await waitForVercelProductionDeployment({
    token, project: PROJECT_NAME, target: 'products', account: teamId ?? 'personal',
