...
WT="$ROOT/.local/asol-final-retry-${GITHUB_RUN_ID:-manual}-$$"
...
# Remove every temporary repair artifact before policy/architecture checks.
rm -f .github/workflows/asol-finalize-repair.yml .github/workflows/asol-long-repair.yml
rm -f scripts/asol-long-repair.sh scripts/asol-long-repair-resume.sh scripts/asol-finalize-repair.sh scripts/asol-finalize-retry.sh
...
commit_push "make iOS push validation hermetic and remove temporary repair tooling"
