...
import storageProfiles from "../src/config/storage-profiles.json";
...
async function r2(): Promise<R2Module> {
  if (!r2Module) {
    r2Module = await import("../src/core/provisioning/r2-s3-client");
  }
  return r2Module;
}
...
async function tursoExecuteWithTimeout(
  client: Client,
  statement: Parameters<Client["execute"]>[0],
) {
