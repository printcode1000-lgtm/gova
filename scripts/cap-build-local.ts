...
function main(): void {
  ...
  console.log(
    `Building local Capacitor assets with existing version ${version}...`,
  );
  console.log(
  ...
  console.log(
    `Local Capacitor build completed at version ${version}. Nothing was published to R2.`,
  );
}
...
try {
  ...
  console.error(
    `Local cap-build failed: ${error instanceof Error ? error.message : error}`,
  );
  process.exitCode = 1;
}
