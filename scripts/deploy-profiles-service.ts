...
/**
 * Deploys the profiles service to its own Vercel account.
 *
 * Three constraints shape this script:
 *
 * 1. Only `services/profiles` is uploaded. The service imports shared
 ...
 *    Nothing here touches its link: the CLI runs with `services/profiles`
...
const PROJECT_NAME = 'asol-profiles';
const SERVICE_DIR = path.join(process.cwd(), 'services', 'profiles');

const token = process.env.VERCEL_PROFILES_TOKEN;
if (!token) {
  console.error('VERCEL_PROFILES_TOKEN is missing from .env / .env.local');
  process.exit(1);
}
...
/** The remaining six shards, plus the image bucket avatars are served from. */
const OPTIONAL_ENV_KEYS = [
  'PROFILE_CONTACT_DATABASE_URL','PROFILE_CONTACT_DATABASE_AUTH_TOKEN',
  'PROFILE_MEDIA_DATABASE_URL','PROFILE_MEDIA_DATABASE_AUTH_TOKEN',
  'PROFILE_SOCIAL_DATABASE_URL','PROFILE_SOCIAL_DATABASE_AUTH_TOKEN',
  'PROFILE_CATALOG_DATABASE_URL','PROFILE_CATALOG_DATABASE_AUTH_TOKEN',
  'PROFILE_PROMOTIONS_DATABASE_URL','PROFILE_PROMOTIONS_DATABASE_AUTH_TOKEN',
  'PROFILE_FULFILLMENT_DATABASE_URL','PROFILE_FULFILLMENT_DATABASE_AUTH_TOKEN',
...
async function main(): Promise<void> {
  ...
  console.log('\nMirroring shared modules into services/profiles/generated...');
  syncSharedSources();

  console.log('\nUploading services/profiles and building remotely...');
  const runId = process.env.ASOL_DEPLOYMENT_RUN_ID?.trim() || `profiles-${Date.now()}`;
  const revision = process.env.ASOL_DEPLOYMENT_REVISION?.trim() || 'standalone';
  const comment = process.env.ASOL_DEPLOYMENT_COMMENT?.trim() || `profiles production deploy ${new Date().toISOString()}`;
  runVercel(['deploy', '--prod', '--yes', ...vercelDeploymentMetadata({ target: 'profiles', comment, runId, revision })], projectId, teamId);
  const report = await waitForVercelProductionDeployment({
    token, project: PROJECT_NAME, target: 'profiles', account: teamId ?? 'personal',
  ...
  console.log(
    "\nDone. Set NEXT_PUBLIC_ASOL_PROFILES_URL on the main app to this deployment's " +
      'origin so the browser bridge sends profile reads there.',
  );
}
