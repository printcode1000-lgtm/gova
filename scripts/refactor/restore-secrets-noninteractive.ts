/**
 * Non-interactive secrets restore for cloud agents (passphrase via argv).
 * Mirrors scripts/restore-project-secrets.ts without the TTY prompt.
 */
...
import type { SecretArchiveManifest } from '@asol/secrets-core';
...
async function main(): Promise<void> {
  ...
  if (!password) {
    throw new Error('Usage: npx tsx scripts/refactor/restore-secrets-noninteractive.ts <passphrase>');
  }
