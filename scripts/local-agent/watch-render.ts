...
export type PanelKey = "runners" | "agents" | "locks" | "running" | "finished" | "messages" | "requests";
...
function renderRunning(model: WatchModel, options: RenderOptions): string[] {
  ...
  if (model.running.length === 0) lines.push(paint("  no mutation running", "dim", options.color));
  for (const operation of model.running) {
    lines.push(
      `  ${pad(operation.agentId, 22)} ${pad(operation.targetRef, 34)} ${pad(operation.runnerName ?? "—", 14)} ` +
...
function renderFooter(options: RenderOptions): string {
  const keys = options.focus
    ? `[esc] all panels  [p] ${options.paused ? "resume" : "pause"}  [o] github on/off  [q] quit`
    : `[1-7] focus panel  [p] ${options.paused ? "resume" : "pause"}  [o] github on/off  [q] quit`;
