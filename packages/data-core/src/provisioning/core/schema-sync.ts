...
import { loadTursoCredentialsFromEnv, loadTursoAdvertisementsCredentialsFromEnv, loadTursoNotificationsCredentialsFromEnv, loadTursoProductCredentialsFromEnv } from './turso-provisioner';
...
export async function runSchemaSync(options: RunSchemaSyncOptions = {}): Promise<SchemaSyncReport> {
  ...
  // turns "the DDL was sent" into "the cloud matches the local design", and it is the only place
  // a silently skipped operation — the `already exists` branch, or a statement the provider
  // accepted without effect — can still be caught before the release continues.
  const { operations: residualOperations } = diffSchemas(sqliteSchema, tursoSchemaAfter, {
    ignoredExtraTables: ignoredExtraTablesFor(databaseLabel),
    removeExtraObjects: options.removeExtraObjects,
  });
  ...
  if (residualOperations.length > 0) {
    throw new Error(
      `Schema sync for ${databaseLabel} reported success but ${residualOperations.length} ` +
        `difference(s) remain between the local schema and Turso after the DDL was applied:\n` +
