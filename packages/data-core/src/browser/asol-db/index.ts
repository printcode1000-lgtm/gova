...
export const ASOL_DB_STORES = {
  ...
  PAGE_SAVE_JOURNAL: 'pageSaveJournal',
  FAVORITES: 'favorites',
  CART: 'cart',
...
export async function asolDbGet<T>(storeName: AsolDbStoreName, key: string): Promise<T | null> {
  ...
  return traceBrowserDatabaseOperation(storeName, key, 'get', async () => {
    const db = await getDB();
...
export async function asolDbSet<T>(storeName: AsolDbStoreName, key: string, value: T): Promise<void> {
  ...
  return traceBrowserDatabaseOperation(storeName, key, 'set', async () => {
    const db = await getDB();
...
export async function asolDbDelete(storeName: AsolDbStoreName, key: string): Promise<void> {
  if (!hasIndexedDb()) return;
  return traceBrowserDatabaseOperation(storeName, key, 'delete', async () => {
    const db = await getDB();
    await new Promise<void>((resolve, reject) => {
      const tx = db.transaction(storeName, 'readwrite');
      const store = tx.objectStore(storeName);
...
/** Stores structured-cloneable values such as Blob/File without JSON conversion. */
export async function asolDbSetStructured<T>(
  storeName: AsolDbStoreName,
  key: string,
  value: T,
): Promise<void> {
  if (!hasIndexedDb()) return;
  return traceBrowserDatabaseOperation(storeName, key, 'set', () =>
    retryClosingConnection(async () => {
      const db = await getDB();
      await new Promise<void>((resolve, reject) => {
        const tx = db.transaction(storeName, 'readwrite');
        const store = tx.objectStore(storeName);
...
export async function asolDbGetAll<T>(storeName: AsolDbStoreName): Promise<Array<{ key: string; value: T }>> {
  if (!hasIndexedDb()) return [];
  return traceBrowserDatabaseOperation(storeName, '*', 'get', async () => {
    const db = await getDB();
...
export async function asolDbClearStore(storeName: AsolDbStoreName): Promise<void> {
  if (!hasIndexedDb()) return;
  return traceBrowserDatabaseOperation(storeName, '*', 'clear', async () => {
...
export async function asolDbGetSuperAdminOriginalSession<T>(): Promise<T | null> {
  return asolDbGet<T>(ASOL_DB_STORES.AUTH, SUPER_ADMIN_ORIGINAL_SESSION_KEY);
}

export async function asolDbSetSuperAdminOriginalSession<T>(session: T): Promise<void> {
  return asolDbSet<T>(ASOL_DB_STORES.AUTH, SUPER_ADMIN_ORIGINAL_SESSION_KEY, session);
}
