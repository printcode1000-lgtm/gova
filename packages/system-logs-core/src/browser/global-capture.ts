import type { SystemLogLevel } from '../domain/entities';
import { captureSystemLog } from './capture-coordinator';
import {
...
function isExpectedOtaFilesystemMiss(method: ConsoleMethod, data: unknown[]) {
  ...
  return (
    ...
    /'(?:stat|rmdir)' failed because file at '.+\/asol-ota\/.+' does not exist\./.test(
      message,
    )
  );
}
...
export function installGlobalCapture(options: InstallGlobalCaptureOptions) {
  setMemoryCollectorAuthorized(options.authorized);
  ...
  methods.forEach((method) => {
    console[method] = (...data: unknown[]) => {
      ...
      if (
        shouldIgnoreConsoleEntry(method, data) ||
        isExpectedOtaFilesystemMiss(method, data)
      ) {
        return;
      }
      ...
      void captureSystemLog(
        ...
        { memory: true, persist: level !== 'normal', emitConsole: false },
      );
    };
  });
  ...
  const unsubscribeCrash = systemLogsNativeCrash().onCrash((payload) => {
    const versions = options.versions();
    void captureSystemLog(
      {
        level: 'error',
        source: 'native',
        consoleMethod: 'native.crash',
        message: String(payload.message ?? 'Native crash reported'),
        page: page(),
        platform: platform(),
        errorName: String(payload.name ?? 'NativeCrash'),
        userAgent: navigator.userAgent,
        feature: 'NativeBridge',
        operation: String(payload.operation ?? 'crash'),
  ...
  return () => {
    ...
    unsubscribeCrash();
    setMemoryCollectorAuthorized(false);
  };
}

export { setMemoryCaptureEnabled, setMemoryCollectorAuthorized };
