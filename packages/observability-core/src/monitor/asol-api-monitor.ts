...
import { emitServerTraceFromHeader } from '../traces/emit-server-trace';
...
function shouldTrack(): boolean {
  return typeof window !== 'undefined' && isObservabilityEnabled();
}
...
export async function trackAsolApiRequest<T>(
  ...
  const base: Omit<OperationRecord, 'status' | 'completedAt' | 'executionTime'> = {
    ...
    httpMethod: method,
    httpRoute: route,
    monitorLayer: isBusinessApi ? 'asol-api' : 'cache',
    timestamp: ts,
    startedAt,
    rowsRead: 0,
    rowsWritten: 0,
