...
export const ALLOWED_PROCESS_ENV_FILES = new Set([
  ...
  // Development-only live simulation credentials and proxy trust material.
  "src/core/config/simulation.server.ts",
...
export const ALLOWED_DB_DRIVER_FILES_PATTERN = [/^packages\/data-core\/src\//];

export const ALLOWED_SQL_FILES_PATTERN = [/^packages\/data-core\/src\//];
...
export function classifyLayer(relativePath: string): ArchitectureLayer {
  ...
  if (p.startsWith("packages/data-core/src/provisioning/core/"))
    return "provisioning";
  if (p.startsWith("packages/data-core/src/domains/") && p.includes("/tests/"))
    return "dev-tools";
  if (
    p.startsWith("packages/data-core/src/domains/") &&
  ...
  if (p.startsWith("packages/data-core/src/domains/marketplace-orders/db/"))
    return "database-client";
  if (
    p.startsWith("packages/data-core/src/domains/marketplace-orders/") &&
    p.endsWith("/index.server.ts")
  )
    return "server-services";
  if (
    p.startsWith("packages/data-core/src/domains/") &&
  ...
  if (
    ...
    return "operations";
  if (
    p === "src/features/google-play-console/domain/development-guard.server.ts"
  )
  ...
  if (
    p.includes("/services/") &&
    (p.endsWith("-api-service.ts") ||
      p.endsWith("/auth-service.ts") ||
      p.endsWith("/session-service.ts"))
  ) {
    return "client-services";
  }
  if (p.includes("/hooks/")) return "hooks";
  if (p.startsWith("src/app/api/")) return "business-api";
  if (p.startsWith("src/app/dev/") || p.startsWith("src/features/dev-tools/"))
    return "dev-tools";
  // Shared application UI (design-system primitives, shell, brand) lives under src/shared/.
  if (
    p.startsWith("src/shared/layouts/") ||
    p.startsWith("src/shared/brand/") ||
    p.startsWith("src/shared/ui/")
  ) {
    return "ui";
  }
...
export function importTargetLayer(
  importPath: string,
): ArchitectureLayer | "external" | "forbidden-package" {
  ...
  if (
    importPath === "@asol/data-core" ||
    importPath.startsWith("@asol/data-core/")
  ) {
    const dataCoreDoor = importPath
      .slice("@asol/data-core".length)
      .replace(/^\//, "");
    ...
    if (dataCoreDoor === "provisioning" || dataCoreDoor === "tooling")
      return "provisioning";
  ...
  if (resolved.startsWith("packages/data-core/src/core/"))
    return "database-client";
  if (resolved.startsWith("packages/data-core/src/browser/")) return "shared";
  if (
    resolved.startsWith("packages/data-core/src/domains/marketplace-orders/") &&
    resolved.endsWith("/index.server")
  ) {
    return "server-services";
  }
  if (
  ...
  if (
    resolved.includes("-service.server") ||
    (resolved.includes("/services/") && resolved.endsWith(".server.ts"))
  ) {
    return "server-services";
  }
  if (
    resolved.includes("/services/") &&
    (resolved.includes("-api-service") ||
      resolved.endsWith("/auth-service") ||
      resolved.endsWith("/session-service"))
  ) {
    return "client-services";
  }
  if (
    resolved.includes("/core/database/asol-db-persister") ||
    resolved.startsWith("packages/data-core/src/browser/asol-db/")
  ) {
    return "shared";
  }
  if (
    resolved.includes("/core/database/db-client") ||
    resolved.includes("/core/database/sqlite-db-client") ||
    resolved.includes("/core/database/profile-db-client")
  ) {
    return "database-client";
  }
  if (
    resolved.includes("/core/database/") ||
    resolved === "packages/data-core/src/core/turso/users-turso-client.ts"
  ) {
    return "database-client";
  }
  if (
    resolved.includes("/core/api/asol-api-client") ||
    resolved === "src/core/api"
  )
    return "asol-api-client";
...
const FORBIDDEN_IMPORTS_BY_LAYER: Partial<
  Record<ArchitectureLayer, ArchitectureLayer[]>
> = {
  ui: [
    "repository",
    "operations",
...
export function getForbiddenImportViolation(
  ...
  if (target === "forbidden-package") {
    ...
    if (
      ...
      importerLayer === "database-client" ||
      importerLayer === "provisioning"
    ) {
      return null;
    }
  ...
  if (importerLayer === "server-services" && target === "operations")
    return null;
  if (importerLayer === "server-services" && target === "server-services")
    return null;
  if (importerLayer === "operations" && target === "repository") return null;
  if (importerLayer === "business-api" && target === "server-services")
    return null;
  if (importerLayer === "hooks" && target === "client-services") return null;
  if (importerLayer === "ui" && target === "hooks") return null;
  if (importerLayer === "client-services" && target === "asol-api-client")
    return null;
  if (importerLayer === "asol-api-client" && target === "asol-http-transport")
    return null;
  if (importerLayer === "asol-api-client" && target === "configuration")
    return null;
  if (
    importerLayer === "dev-tools" &&
    (target === "asol-api-client" || target === "configuration")
  )
    return null;
  if (
