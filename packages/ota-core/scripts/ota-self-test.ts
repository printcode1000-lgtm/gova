...
import type { OtaManifestPayload } from "../src";
...
async function main(): Promise<void> {
  ...
  const payload: OtaManifestPayload = {
    ...
    requiredCapabilities: ["barcode.scan"],
    mandatory: false,
    notes: "self-test",
    files: {
      "index.html": {
  ...
  if (!signatureValid)
    throw new Error("P-256 manifest signature verification failed");

  const checksum = createHash("sha256").update(indexHtml).digest("hex");
  if (
    checksum.length !== 64 ||
    checksum !== payload.files["index.html"]!.sha256
  ) {
    throw new Error("File SHA-256 self-test failed");
  }
  if (
    getOtaPublicKeyBase64(privateKey) !==
...
main().catch((error) => {
  console.error(
    `OTA self-test failed: ${error instanceof Error ? error.message : error}`,
  );
  process.exit(1);
});
