...
const nativePlatform = {
  ...
  backgroundDownload: {
    ...
    status: async (options: any): Promise<BackgroundDownloadTask> => {
      const id = typeof options === "string" ? options : options?.releaseId;
      const res = await NativeCore.getBackgroundDownloadStatus(id);
      if (!res.ok) throw new Error(res.error.message);
      return res.value ?? { id: id ?? "", url: "", destinationPath: "", bytesDownloaded: 0, totalBytes: 0, state: "missing" as const, status: "missing" as const };
    },
    ...
    read: async (task: any, onChunk: (chunk: Uint8Array) => Promise<void> | void) => {
      if (task.destinationPath) {
        const res = await NativeCore.otaReadReleaseFile(task.version ?? "current", task.destinationPath);
        if (res.ok) {
