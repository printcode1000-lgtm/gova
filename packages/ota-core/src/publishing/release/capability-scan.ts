...
const apiPatterns = new Map<string, string>([
  ...
  ["NativeCore.otaNativeVersion", CapabilityKeys.AppState],
  ["NativeCore.otaActivate", CapabilityKeys.AppState],
  ["NativeCore.otaRevertToNativeBaseline", CapabilityKeys.AppState],
]);
...
export function assertDetectionCoverage(): void {
  ...
  const undetectable = (ALL_CAPABILITY_KEYS as readonly string[]).filter(
    (key: string) => !detectable.has(key as CapabilityKey),
  );
  if (undetectable.length > 0) {
    throw new Error(
      "Capability keys with no detection pattern in capability-scan.ts:\n" +
        undetectable.map((key: string) => `  - ${key}`).join("\n") +
