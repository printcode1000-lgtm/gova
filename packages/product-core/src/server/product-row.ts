import { createEmptyProductDetails } from "../domain/product.entity";
import type {
  ProductDetails,
  ProductImage,
  ProductRatingMode,
...
import { normalizeRatingMode } from "../domain/normalize";
...
export const MUTABLE_PRODUCT_COLUMNS = PRODUCT_COLUMNS.filter(
  (column) =>
    !["id", "uid", "main_category_id", "subcategory_id", "created_at"].includes(
      column,
    ),
);
  ...
  return {
    ...
    ...createEmptyProductDetails({
      ...
      rating: {
        ...
        mode: normalizeRatingMode(row.rating_mode) as ProductRatingMode,
      },
      images: parseProductImages(row.images_json),
    }),
  };
}

export function productRowValues(record: ProductRecord): unknown[] {
  return [
    record.id,
    record.uid,
    ...
    record.rating.mode,
    serializeProductImages(record.images),
    record.status,
    record.createdAt,
    record.updatedAt,
  ];
