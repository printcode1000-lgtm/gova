...
import {
  ...
  type DevCloudBackupEnvironment,
} from "../domain/types";
import { r2BackupRepository } from "./r2-backup.repository";
export type {
  BackupDatabasePort,
  BackupDatabaseSource,
  DevCloudBackupPorts,
} from "./dev-cloud-backup-ports";
import type { DevCloudBackupPorts } from "./dev-cloud-backup-ports";
...
export class DevCloudBackupService {
  constructor(private readonly ports: DevCloudBackupPorts) {}

  environment() {
    return this.ports.environment();
  }

  async list(): Promise<{ environment: DevCloudBackupEnvironment; backups: DevCloudBackupListItem[] }> {
  ...
  private async buildArchive(
    ...
    for (const source of this.ports.database.sources()) {
      const backup = await this.ports.database.exportDatabase(source);
  ...
  async restore(
    ...
    for (const database of manifest.databases) {
      ...
      const result = await this.ports.database.restoreDatabase(database, files, mode);
      restoredDatabases += 1;
