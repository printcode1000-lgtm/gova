 ...
 * tag name); source shows the component that produced it. This table is the
 * only bridge between the two, and it is deliberately explicit: guessing a
 * component from a tag name is how tooling ends up editing the wrong file.
 */
const COMPONENTS_BY_MARKER: Readonly<Record<string, readonly string[]>> = {
  button: ["Button"],
  input: ["Input", "FormInput"],
  textarea: ["Textarea", "FormTextarea"],
  switch: ["Switch"],
  checkbox: ["Checkbox"],
  "select-trigger": ["SelectTrigger", "FormSelect", "PharmacySelect"],
...
export function renderUiDescriptorProp(descriptor: Record<string, unknown>): string {
  const order = ["uid", "id", "kind", "action", "part", "state", "simulation"];
  const fields = order
