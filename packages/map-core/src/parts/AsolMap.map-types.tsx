...
/**
 * A refused location permission arrives here as an ordinary rejection, so it would be
 * reported as a generic `gps` failure and the host would have no way to tell "ask
 * again" from "only the settings screen can fix this". The permission error carries
 * that state, so it is promoted to its own code and the state travels with it.
 */
const isPermissionError = (
  cause: unknown,
): cause is Error & {
  state: NonNullable<AsolMapError["permissionState"]>;
  requiresSettings: boolean;
} =>
  cause instanceof Error &&
  cause.name === "MapLocationPermissionError" &&
  "state" in cause;
...
export const error = (
  ...
  isPermissionError(cause)
    ? {
        ...
        permissionState: cause.state,
        requiresSettings: cause.requiresSettings,
      }
    : {
        code,
        cause,
