...
import { isNotificationWebViewForeground } from "../notifications/delivery-visibility";
import {
  PermissionStates,
  fromCapacitorState,
  toPermissionResult,
...
// ---------------------------------------------------------------------------
// Lazy plugin loading -- transient startup failures must be retryable
// ---------------------------------------------------------------------------
{
  let attempts = 0;
  const lazy = createLazyPlugin("RetryablePlugin", async () => {
    attempts += 1;
    if (attempts === 1) throw new Error("bridge not ready yet");
    return { ready: true };
  });

  assert.equal(await lazy.optional(), null);
  assert.equal(lazy.isLoaded(), false);
  assert.deepEqual(await lazy.required(), { ready: true });
  assert.equal(attempts, 2, "A transient plugin failure was cached forever.");
  assert.equal(lazy.isLoaded(), true);
}

// ---------------------------------------------------------------------------
// Push presentation visibility
// ---------------------------------------------------------------------------
{
  assert.equal(
    isNotificationWebViewForeground({ visibilityState: "visible" }),
    true,
  );
  assert.equal(
    isNotificationWebViewForeground({ visibilityState: "hidden" }),
    false,
    "A background Android delivery would be re-presented locally.",
  );
  assert.equal(isNotificationWebViewForeground(undefined), false);
}
