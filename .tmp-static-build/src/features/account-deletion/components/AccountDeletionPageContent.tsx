...
const COPY = {
  ...
  en: {
    ...
    phrase: "Type this phrase exactly",
    acknowledge:
    ...
    invalid: "Check the password and confirmation phrase, then try again.",
...
export function AccountDeletionPageContent() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!session || !accepted || phrase !== ACCOUNT_DELETION_PHRASE) {
      setState("error");
      return;
    }
    setState("deleting");
    ...
    >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {c.items.map((item, index) => {
            ...
            return (
              <div
                key={item}
                className="flex gap-3 rounded-xl bg-surface-container p-4"
              >
      ...
      >
        ...
        <label className="block space-y-2 font-semibold">
          ...
          <code
            dir="ltr"
            className="block rounded-xl bg-surface-container p-3 text-center text-base"
          >
        ...
        {state === "error" && (
          <p className="rounded-xl bg-red-100 p-3 text-red-800">{c.invalid}</p>
        )}
        <button
          disabled={
            !accepted ||
      ...
      <Link
        href="/contact-us"
        className="block text-center font-semibold text-primary"
      >
        {c.contact}
      </Link>
    </main>
