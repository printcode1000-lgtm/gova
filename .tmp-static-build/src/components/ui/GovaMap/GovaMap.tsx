...
const SOURCE = {
  markers: "gova-markers",
  polygons: "gova-polygons",
  circles: "gova-circles",
  routes: "gova-routes",
  heat: "gova-heat",
  custom: "gova-geojson",
} as const;
...
export const GovaMap = forwardRef<GovaMapHandle, GovaMapProps>(
  function GovaMap(props, ref) {
    ...
    const emitError = useCallback((value: GovaMapError) => {
      ...
      try {
        ...
        map.on("click", (event) => {
          const features = map.queryRenderedFeatures(event.point, {
            layers: ["gova-marker", "gova-cluster"],
          });
          ...
          if (feature?.layer.id === "gova-marker") {
            ...
            if (
              ...
              element.className = "gova-map__popup";
    ...
    const viewport = (): GovaMapViewport => {
      const map = mapRef.current;
    ...
    return (
      <section
        id={props.id}
        className={`gova-map ${theme.className ?? ""} ${props.className ?? ""}`}
        style={props.style}
      ...
      >
        <div
          ref={containerRef}
          className="gova-map__canvas"
          role="application"
          aria-label={props.ariaLabel ?? "Interactive map"}
          tabIndex={0}
        />
        {present(props.layers?.controls) && (
          <GovaMapControls config={props.toolbar} onAction={control} />
        )}
        {props.children}
        {status === "loading" && (
          <div className="gova-map__overlay" role="status">
            <span className="gova-map__spinner" aria-hidden="true" />
            <span>{props.loadingLabel ?? "Loading map…"}</span>
          </div>
        )}
        {status === "error" && (
          <div className="gova-map__overlay" role="alert">
            <p>{fatal?.message ?? "The map could not be loaded."}</p>
            <button
              className="gova-map__retry"
              type="button"
              onClick={() => {
                mapRef.current?.remove();
                mapRef.current = null;
