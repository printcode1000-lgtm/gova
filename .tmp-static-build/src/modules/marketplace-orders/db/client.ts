...
const rowsOrChanges=(result:{rows:unknown[];rowsAffected:number})=>result.rows.length>0?result.rows as Record<string,unknown>[]:[{changes:result.rowsAffected}];
export function createMarketplaceOrdersDb():MarketplaceDb {
  ...
  const api:MarketplaceDb={async execute(sql,params=[]){const r=await client.execute({sql,args:normalize(params)});return rowsOrChanges(r)},async transaction(work){const tx=await client.transaction("write");const scoped:MarketplaceDb={execute:async(sql,params=[])=>{const r=await tx.execute({sql,args:normalize(params)});return rowsOrChanges(r)},transaction:async fn=>fn(scoped)};try{const result=await work(scoped);await tx.commit();return result}catch(error){await tx.rollback();throw error}}};return api;
}
