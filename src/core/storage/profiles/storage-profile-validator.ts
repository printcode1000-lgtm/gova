...
import type { StorageProfile, StorageProfilesFile } from '../types/storage-profile.types';
...
const ALLOWED_PROVIDERS = new Set(['CloudflareR2', 'GoogleDrive', 'LocalStorage']);

function assertRequiredKeys(profile: Record<string, unknown>, index: number): void {
  for (const key of REQUIRED_PROFILE_KEYS) {
    if (profile[key] === undefined || profile[key] === null) {
      throw new Error(`storage-profiles.json profiles[${index}] missing required property: ${key}`);
    }
  }
}
...
function validateProfile(profile: StorageProfile, seenIds: Set<string>): void {
  if (seenIds.has(profile.id)) {
    throw new Error(`storage-profiles.json duplicate profile id: ${profile.id}`);
  }
  ...
  if (!ALLOWED_PROVIDERS.has(profile.provider)) {
    throw new Error(`Profile "${profile.id}": invalid provider "${profile.provider}"`);
  }

  if (typeof profile.maxImageSizeKB !== 'number' || profile.maxImageSizeKB <= 0) {
    throw new Error(`Profile "${profile.id}": maxImageSizeKB must be a positive number`);
  }

  if (!isAllowedOutputFormat(profile.outputFormat)) {
    throw new Error(`Profile "${profile.id}": invalid outputFormat "${profile.outputFormat}"`);
  }
...
export function validateStorageProfilesFile(raw: unknown): StorageProfilesFile {
  ...
  if (!Array.isArray(file.profiles) || file.profiles.length === 0) {
    throw new Error('storage-profiles.json must contain a non-empty profiles array');
  }
  ...
  file.profiles.forEach((entry, index) => {
    if (!entry || typeof entry !== 'object') {
      throw new Error(`storage-profiles.json profiles[${index}] must be an object`);
    }
