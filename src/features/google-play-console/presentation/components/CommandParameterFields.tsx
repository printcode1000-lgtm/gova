...
export function Doc({ label, value }: { label: string; value: string }) {
  return <div id="google-play-console-presentation-components-commandparameterfields-div-1-v8jbyi"><strong id="google-play-console-presentation-components-commandparameterfields-strong-2-bxjoxu">{label}</strong><p id="google-play-console-presentation-components-commandparameterfields-text-3-l5cibq" className="text-on-surface-variant">{value}</p></div>;
}

/** Extra guidance shown only for the `cap-build` command. */
export function CapBuildGuide({ t }: { t: (key: string) => string }) {
  return <div id="google-play-console-presentation-components-commandparameterfields-div-4-6nag5n" className="space-y-2 border-s-4 border-primary bg-muted/40 p-3 text-sm">
    <h3 id="google-play-console-presentation-components-commandparameterfields-heading-5-earmdg" className="font-semibold">
      {t("releaseConsole.capBuild.guideTitle")}
    </h3>
    <p id="google-play-console-presentation-components-commandparameterfields-text-6-drpj8y" className="text-on-surface-variant">{t("releaseConsole.capBuild.guideBody")}</p>
  </div>;
}
