    ...
    <div {...uiAttributes({ uid: "google-play-console.delta-table.div-NcPi69", id: "google-play-console.delta-table.div" })} id={id} className="overflow-auto rounded-md border">
      <table {...uiAttributes({ uid: "google-play-console.delta-table.table-4Hu4DF", id: "google-play-console.delta-table.table" })} className="w-full min-w-[40rem] text-sm" dir="ltr">
        ...
        <tbody {...uiAttributes({ uid: "google-play-console.delta-table.tbody-3D5aL2", id: "google-play-console.delta-table.tbody" })}>
          {rows.map((row) => (
            <tr key={row.id} {...uiAttributes({ uid: "google-play-console.delta-table.tr.2-RZF0QM", id: "google-play-console.delta-table.tr.2" , instance: createOpaqueUiInstanceId("iter-ddcb0d1494", String(row.id))})} className="border-t">
              <td {...uiAttributes({ uid: "google-play-console.delta-table.td-O0zu5X", id: "google-play-console.delta-table.td" , instance: createOpaqueUiInstanceId("iter-6c219a5aa3", String(row.id))})} className="p-2">{row.id}</td><td {...uiAttributes({ uid: "google-play-console.delta-table.td.2-8tcPpZ", id: "google-play-console.delta-table.td.2" , instance: createOpaqueUiInstanceId("iter-6c219a5aa3", String(row.id))})} className="p-2">{row.leftCompressedBytes}</td>
              <td {...uiAttributes({ uid: "google-play-console.delta-table.td.3-P7YaB7", id: "google-play-console.delta-table.td.3" , instance: createOpaqueUiInstanceId("iter-ed02ca97cd", String(row.id))})} className="p-2">{row.rightCompressedBytes}</td>
              <td {...uiAttributes({ uid: "google-play-console.delta-table.td.4-10Vqt5", id: "google-play-console.delta-table.td.4" , instance: createOpaqueUiInstanceId("iter-704ddf07b0", String(row.id))})} className="p-2">{row.compressedDeltaBytes}</td>
            </tr>
          ))}
