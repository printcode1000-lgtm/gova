...
export function DevCloudBackupPage() {
  ...
  if (page.isLoading) {
    return <main id="dev-cloud-backup.dev-cloud-backup-page.main" className="p-4 text-sm text-on-surface-variant">جاري التحميل...</main>;
  }

  if (!page.allowedUser) {
    return (
      <main id="dev-cloud-backup.dev-cloud-backup-page.main.2" className="mx-auto max-w-2xl p-6">
        <div id="dev-cloud-backup.dev-cloud-backup-page.div" className="rounded-md border border-red-200 bg-red-50 p-4 text-red-700">
  ...
  return (
    <main id="dev-cloud-backup.dev-cloud-backup-page.main.3" className="mx-auto w-full max-w-7xl space-y-4 p-4 pb-24" dir="rtl">
      <header id="dev-cloud-backup.dev-cloud-backup-page.header" className="flex flex-wrap items-start justify-between gap-3">
        <div id="dev-cloud-backup.dev-cloud-backup-page.div.2">
          <h1 id="dev-cloud-backup.dev-cloud-backup-page.h1" className="flex items-center gap-2 text-2xl font-semibold text-on-surface">
            <DatabaseBackup id="dev-cloud-backup.dev-cloud-backup-page.database-backup" className="h-6 w-6 text-primary" />
            نسخ سحابة التطوير
          </h1>
          <p id="dev-cloud-backup.dev-cloud-backup-page.p" className="mt-1 text-sm text-on-surface-variant">
        ...
        <Button id="dev-cloud-backup.dev-cloud-backup-page.button" type="button" variant="outline" onClick={() => void page.load()} disabled={page.busy === "load"}>
      ...
      {!devAllowed ? (
        <section id="dev-cloud-backup.dev-cloud-backup-page.section" className="rounded-md border border-amber-200 bg-amber-50 p-4 text-amber-800">
          <div id="dev-cloud-backup.dev-cloud-backup-page.div.3" className="flex items-center gap-2 font-semibold">
            <ShieldAlert id="dev-cloud-backup.dev-cloud-backup-page.shield-alert" className="h-5 w-5" />
            المديول مقفل خارج بيئة التطوير
          </div>
          <p id="dev-cloud-backup.dev-cloud-backup-page.p.2" className="mt-1 text-sm">
      ...
      {page.error ? (
        <section id="dev-cloud-backup.dev-cloud-backup-page.section.2" className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {page.error}
        </section>
      ) : null}
      {page.notice ? (
        <section id="dev-cloud-backup.dev-cloud-backup-page.section.3" className="rounded-md border border-green-200 bg-green-50 p-3 text-sm text-green-700">
          {page.notice}
        </section>
      ) : null}
      ...
      <section id="dev-cloud-backup.dev-cloud-backup-page.section.4" className="grid gap-4 lg:grid-cols-[1fr_1fr]">
