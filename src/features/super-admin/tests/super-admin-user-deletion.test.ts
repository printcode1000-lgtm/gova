...
assert.match(deletionRepositorySource, /id AS order_id FROM orders WHERE buyer_id = \?/);
assert.match(deletionRepositorySource, /order_id FROM seller_orders WHERE seller_id = \? OR service_provider_id = \?/);
assert.match(deletionRepositorySource, /order_id FROM shipments WHERE carrier_id = \?/);
assert.match(deletionRepositorySource, /FROM orders WHERE id = \?/);
assert.doesNotMatch(deletionRepositorySource, /orders SET delivery_address_snapshot_json/);
