...
import { actorFromInput, moneyMinor } from "@asol/orders-core";
import {
  notificationsServer,
 ...
 *
 * This was the body of `POST /api/orders/:orderId/actions` — six hundred lines that built four
 * actor identities, resolved notification recipients, dispatched forty actions and signed the
 * grants each one earns. None of that is HTTP. The route reads a body, hands it over, and turns
 * a thrown error into a status; the orchestration belongs with the feature that owns orders,
 * where it can be called from something other than a URL.
 *
 * Returns the response payload and throws the domain's own error codes for the route to map.
 */
export async function executeOrderAction(orderId: string, body: ActionInput): Promise<unknown> {
