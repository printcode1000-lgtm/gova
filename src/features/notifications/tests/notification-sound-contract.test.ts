...
for (const channel of DEFAULT_CHANNELS) {
  if (audibleChannels.has(channel.id)) {
    assert.match(
      channel.name,
      /نغمة مخصصة/,
      `Channel ${channel.id} must identify the custom tone in Android settings.`,
    );
    assert.equal(
      channel.sound,
      ANDROID_SOUND_FILE,
...
// The sound URI must name the resource. A channel outlives the install that
// created it and its sound can never be changed, while a numeric resource id is
// regenerated by every build — persisting one inside a channel is a sound that
// breaks on some later upgrade with no way back.
assert.ok(
  androidBootstrap.includes('context.getPackageName() + "/raw/" + SOUND_RESOURCE_NAME'),
  "The native channel sound URI must address the raw resource by name.",
);
assert.doesNotMatch(
  androidBootstrap,
  /getPackageName\(\)\s*\+\s*"\/"\s*\+\s*retainedResource/,
  "The channel sound URI must not embed a build-generated numeric resource id.",
);
assert.doesNotMatch(
  androidBootstrap,
  /SCHEME_ANDROID_RESOURCE[^;]*\+\s*(retainedResource|R\.raw\.)/,
  "No numeric resource id may be concatenated into the sound URI.",
);
assert.ok(
  androidBootstrap.includes("getResourceEntryName(retainedResource)"),
  "The native side must verify the raw resource still carries the name the URI uses.",
);
assert.ok(
  /SOUND_RESOURCE_NAME\s*=\s*"custom_notification"/.test(androidBootstrap),
  "The native sound resource name must be the file the payloads name.",
);
assert.equal(
  FCM_SOUND_RESOURCE,
  "custom_notification",
  "The FCM resource name and the native channel URI must name one asset.",
);
