...
export class DeviceTokenService {
  async register(uid: string): Promise<DeviceToken | null> {
    ...
    if (token) {
      await govaNotificationRepository.saveDeviceToken(token);
  ...
  list(uid: string): Promise<DeviceToken[]> {
    return govaNotificationRepository.listDeviceTokens(uid);
  }

  async remove(uid: string, tokenId: string): Promise<void> {
    await govaNotificationRepository.removeDeviceToken(uid, tokenId);
