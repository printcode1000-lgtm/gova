...
import { toStorageProfileClientView, getStorageProfileById } from '@/core/storage/profiles/storage-profile-loader.server';
...
export class ImageUploadApplicationService {
  ...
  upload(command: UploadImageCommand): Promise<ImageUploadResult> {
    return traceServerLayer('server-service', 'ImageUploadApplicationService.upload', () =>
      imageStorageOrchestrator.upload(command)
    );
  }

  deleteImage(storageProfileId: string, imageKey: string): Promise<void> {
    return traceServerLayer('server-service', 'ImageUploadApplicationService.deleteImage', () =>
      imageStorageOrchestrator.deleteByKey(storageProfileId, imageKey)
    );
  }
}

export const imageUploadApplicationService = new ImageUploadApplicationService();
