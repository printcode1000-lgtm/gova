...
function PasswordField({ id, label, value, onChange, visible, toggle }: { label: string; value: string; onChange: (value: string) => void; visible: boolean; toggle?: () => void } & { id?: string }) {
  return <label id={id} className="space-y-2 block"><span id={id ? `${id}-text-43-siilck` : undefined} className="text-sm font-semibold flex items-center gap-2"><KeyRound className="h-4 w-4 text-primary" />{label}</span><div id={id ? `${id}-div-44-o8nvvv` : undefined} className="relative"><input id={id ? `${id}-input-45-po1zdo` : undefined} type={visible ? 'text' : 'password'} value={value} onChange={(e) => onChange(foldPasswordDigits(e.target.value))} autoComplete="new-password" className="auth-input w-full pe-10" />{toggle && <button id={id ? `${id}-button-46-lrhoob` : undefined} type="button" onClick={toggle} className="absolute end-0 top-0 h-full px-3 text-on-surface-variant">{visible ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>}</div></label>;
}
