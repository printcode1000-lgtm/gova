...
export function DataHealthTopologyPanel({ report }: { report: DataHealthReport }) {
  ...
  return (
    <section className="overflow-hidden rounded-md border bg-surface" dir="rtl">
      ...
      <div className="grid divide-y lg:grid-cols-2 lg:divide-x lg:divide-y-0" dir="rtl">
        <div className="p-3">
          <TopologyDatabaseGroup label="قواعد النظام الأساسية" items={coreDatabases} />
          <TopologyDatabaseGroup label={`قواعد ملفات التعريف (${profileShards.length})`} items={profileShards} />
          <TopologyDatabaseGroup label={`قواعد الطلبات (${orderShards.length})`} items={orderShards} />
        </div>
        ...
        <div className="p-3">
          ...
          <div className="mt-4">
            <div className="mb-2 text-sm font-semibold">كل مصادر الصور المفحوصة</div>
