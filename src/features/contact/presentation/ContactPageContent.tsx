...
function Field({ id, icon, label, children }: { icon: IconDefinition; label: string; children: ReactNode } & { id?: string }) {
  return <label id={id} className="block space-y-2 text-sm font-semibold"><span id={id ? `${id}-text-57-yjobir` : undefined} className="flex items-center gap-2"><FontAwesomeIcon icon={icon} className="text-primary" />{label}</span><span id={id ? `${id}-text-58-dciidj` : undefined} className="block [&>input]:w-full [&>input]:rounded-xl [&>input]:border [&>input]:border-outline/40 [&>input]:bg-surface-container [&>input]:p-3 [&>select]:w-full [&>select]:rounded-xl [&>select]:border [&>select]:border-outline/40 [&>select]:bg-surface-container [&>select]:p-3 [&>textarea]:w-full [&>textarea]:rounded-xl [&>textarea]:border [&>textarea]:border-outline/40 [&>textarea]:bg-surface-container [&>textarea]:p-3">{children}</span></label>;
}

function ContactLine({ id, icon, text, href }: { icon: IconDefinition; text: string; href?: string } & { id?: string }) {
  const body = <><span id={id ? `${id}-text-59-4dgtl6` : undefined} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary"><FontAwesomeIcon icon={icon} /></span><span id={id ? `${id}-text-60-xfb9mt` : undefined}>{text}</span></>;
  return href ? <a id={id} href={href} className="mb-3 flex items-center gap-3 text-sm">{body}</a> : <div id={id ? `${id}-div-62-fodlyn` : undefined} className="mb-3 flex items-center gap-3 text-sm">{body}</div>;
}
