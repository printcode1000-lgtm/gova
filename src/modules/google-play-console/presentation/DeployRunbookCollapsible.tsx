      ...
      >
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <span className={cn("font-semibold break-words", props.nested ? "text-sm" : "text-base")}>{props.title}</span>
            {props.badge}
          </div>
          {props.description ? <p className="text-xs text-on-surface-variant break-words">{props.description}</p> : null}
        </div>
