  ...
  return (
    <aside className="min-w-0 w-full space-y-3">
      <header className="rounded-md border bg-surface p-3 sm:p-4">
        ...
        <p className="mt-1 text-xs text-on-surface-variant">شغّل أو أوقف job النظام المحلي بعد مراجعة الأمر وعبارة التأكيد.</p>
      </header>

      <DeployRunbookCollapsible title="معاينة الأمر" description="الأمر الذي سيُنفَّذ فعلياً عبر job النظام.">
        <div className="min-w-0 overflow-x-auto rounded-md bg-muted p-3 text-xs">
          <code className="block min-w-0 whitespace-pre-wrap break-all text-left" dir="ltr">{props.commandPreview}</code>
          <p className="mt-2 break-words text-on-surface-variant">راجع المعاينة قبل التشغيل؛ السجل الكامل يظهر في الطرفية.</p>
        </div>
      </DeployRunbookCollapsible>

      <DeployRunbookCollapsible title="خيارات الأمان" description="تجاوزات حساسة للنشر والـ manifest والملفات المؤقتة.">
      ...
      <DeployRunbookCollapsible title="تشغيل وإيقاف" description="عبارة التأكيد مطلوبة قبل بدء أي نشر إنتاجي.">
        <div className="space-y-3">
          ...
          {props.startError ? <p className="rounded-md bg-error-container p-2 text-sm text-on-error-container break-words">{props.startError}</p> : null}
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
            <Button className="w-full sm:w-auto" disabled={props.locked || props.confirmation !== props.exactPhrase} onClick={props.onStart}>
...
function ConfirmationPhraseHelp(props: { exactPhrase: string; onApply: (value: string) => void }) {
  ...
  return (
    <div className="asol-selectable flex flex-wrap items-start justify-between gap-2 text-xs text-on-surface-variant">
      ...
      <Button type="button" variant="outline" size="sm" className="h-7 shrink-0 px-2" onClick={applyPhrase}>
