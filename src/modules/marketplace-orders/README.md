...
Authenticated server routes use the typed facade exported by `@/modules/data-access/domains/marketplace-orders/index.server` and pass the authenticated `{id, role}` actor. The facade wraps every public operation in a database transaction without exposing a database client. Data-backed examples and tests live with the data-access domain; `npm run test:marketplace-orders` executes coverage of multi-seller/mixed/custom image orders, pricing offers, transport roll-up, duplicate-assignment rejection, partial/full delivery, payment, partial refund, return, dispute, permissions and audit creation.
