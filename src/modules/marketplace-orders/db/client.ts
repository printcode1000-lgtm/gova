...
import { isReadQuery, rowsOrChanges } from "./result-mapper";
...
export function createMarketplaceOrdersDb():MarketplaceDb {
  ...
  const api:MarketplaceDb={async execute(sql,params=[]){const r=await client.execute({sql,args:normalize(params)});return rowsOrChanges(sql,r)},async transaction(work){const tx=await client.transaction("write");const scoped:MarketplaceDb={execute:async(sql,params=[])=>{const r=await tx.execute({sql,args:normalize(params)});return rowsOrChanges(sql,r)},transaction:async fn=>fn(scoped)};try{const result=await work(scoped);await tx.commit();return result}catch(error){await tx.rollback();throw error}}};return api;
}
