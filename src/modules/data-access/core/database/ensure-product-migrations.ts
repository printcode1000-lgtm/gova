...
import { isDevRuntime } from '@/modules/data-access/core/database/environment';
...
export function ensureProductDevMigrations(db: { db?: unknown } | unknown): void {
  ...
  migrate(database, {
    migrationsFolder: path.join(process.cwd(), 'src', 'modules', 'data-access', 'core', 'database', 'product', 'migrations'),
  });
  migrationsRun = true;
}
