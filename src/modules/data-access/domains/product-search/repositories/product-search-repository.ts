import { productsDataSource } from "@/modules/data-access/core";
import "server-only";

import {
  PRODUCT_COLUMNS,
  mapProductRow,
} from "@/modules/data-access/domains/product/repositories/product-repository";
import {
...
export class ProductSearchRepository {
  async search(request: ProductSearchRequest): Promise<ProductSearchResult> {
    ...
    const totalRows = (await productsDataSource.execute(
