...
function routeFor(database: string, table: string, recordId: string): string {
  if (table === "products") return `/product?mode=view&id=${encodeURIComponent(recordId)}`;
...
export class DataHealthRepository {
  ...
  async scan(): Promise<DataHealthReport> {
    ...
    try {
      ...
      this.collectOrderHeaderIssues(issues, orders, sellerOrders, customItems, context);
  ...
  private async insertRun(runId: string, startedAt: string) {
    ...
    await profileDbClient.execute(
      ...
      [runId, isProduction ? "production" : "development", startedAt],
    );
  }
  ...
  private async collectProfileIssues(
    ...
    const [images, featured, follows, carriers, reviews, helpful, replies, usages] =
      await Promise.all([
        profileDbClient.execute(
    ...
    for (const objectPath of inventory.objectPaths) {
      ...
      issues.push(
        makeIssue({
          ...
          details: objectPath,
          evidence: { storageProfileId: profileId, imageKey, objectPath },
  ...
  private pushInvalidAdvertisementJson(
    ...
    issues.push(
      makeIssue({
        ...
        evidence: { raw: text(row.config_json ?? row.product_ids_json).slice(0, 500) },
        cleanupAction: "none",
        cleanupMode: "manual",
        updatedAt: text(row.updated_at),
      }),
  ...
  private profileIdForObjectPath(objectPath: string): string {
    ...
    return (
      profiles.find((id) => {
        const profile = this.storageProfile(id);
        return objectPath === profile.folder || objectPath.startsWith(`${profile.folder}/`);
      }) ?? "product-default"
    );
  }
  ...
  private storageProfile(profileId: string) {
    const folders: Record<string, string> = {
      ...
      cover: "images/covers",
      "home-hero-slider": "images/advertisements/home-hero-slider",
      "product-default": "images/products",
      spicialOrder: "images/spicialOrder",
    };
    return { folder: folders[profileId] ?? "images/products" };
  }
  ...
  async executeCleanup(input: {
    ...
    for (const issueId of input.issueIds) {
      ...
      let reason = "";
      try {
        if (!issue || !issue.canClean) throw new Error("issueNoLongerCleanable");
