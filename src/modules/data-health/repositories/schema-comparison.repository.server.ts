...
async function readSchema(executor: Executor): Promise<Map<string, SchemaObject>> {
...
function compareSchemas(
  ...
  for (const [key, localObject] of local) {
    ...
    if (
      localObject.sql !== cloudObject.sql
    ) {
      warnings.push(
...
const SOURCES = [
  {
    label: "users",
    local: () => dbClient as unknown as Executor,
  ...
  },
  {
    ...
    local: () => advertisementsDbClient as unknown as Executor,
    credentials: () =>
      credentials(
