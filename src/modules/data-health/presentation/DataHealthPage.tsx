...
export function DataHealthPage() {
  ...
  }, [authHeaders, report?.schemaComparison.databases.length, schemaLoading, session]);
    ...
    <main className="mx-auto w-full max-w-7xl space-y-4 p-4 pb-24">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-on-surface-variant">
            ...
            {report ? (
              ...
              >
                {report.environment === "production"
                  ? "الإنتاج: Turso وR2"
                  : "التطوير: SQLite والتخزين المحلي"}
              </span>
            ) : null}
          </div>
        </div>
      ...
      {report ? (
        <section className="grid gap-2 sm:grid-cols-3 lg:grid-cols-7">
          ...
          <Summary label="قابل للتنظيف" value={report.summary.cleanable} tone="green" />
          <Summary label="في الحجر" value={report.summary.quarantined} tone="amber" />
        </section>
      ) : null}
        ...
        <TabsContent value="findings" className="space-y-3">
          ...
          <section className="flex flex-wrap items-center justify-between gap-3 rounded-md border bg-surface p-3">
            ...
            >
              ...
              {planning ? "جاري إعداد المعاينة" : `معاينة تنظيف ${selected.length}`}
            </Button>
          </section>
        </TabsContent>
      ...
      <Dialog open={Boolean(plan)} onOpenChange={(open) => !open && setPlan(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto" dir="rtl">
          <DialogHeader>
            ...
            <DialogDescription>
              الخطة صالحة حتى {dateText(plan?.expiresAt)} ولا يمكن استخدامها أكثر
              من مرة.
            </DialogDescription>
          </DialogHeader>
      ...
      {comparison?.databases.map((database) => (
        <div key={database.database} className="rounded-md border bg-surface p-3">
          ...
          {[...database.operations.map((item) => item.description), ...database.warnings]
            .slice(0, 100)
            .map((message, index) => (
              <div key={`${message}-${index}`} className="mt-2 border-t pt-2 text-xs">
                {message}
              </div>
            ))}
        </div>
          ...
          {history.audit.map((entry) => (
            <div key={entry.id} className="p-3 text-sm">
              <div className="flex items-center justify-between gap-2">
                ...
                <span
                  className={
                    entry.status === "cleaned" ? "text-green-700" : "text-amber-700"
                  }
                >
