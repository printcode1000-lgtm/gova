...
`GovaMap` is GoVa's shared, provider-agnostic mapping platform. It uses MapLibre GL JS, GeoJSON, OpenStreetMap-compatible tiles, and Supercluster. It has no Mapbox, Google Maps, paid SDK, or map-engine API-key requirement.
...
`GovaMap` is a client component. A server component may render a client wrapper that contains it. No `ssr: false` dynamic import is required.
...
import { GovaMap, createOpenStreetMapProvider } from '@/components/ui/GovaMap';
...
| --- | --- |
| `GovaMap.tsx` | Map engine lifecycle, event translation, layer orchestration, and recovery UI |
...
| `GovaMapControls.tsx` | Accessible independent controls |
...
| `GovaMap.css` | Responsive, safe-area-aware presentation |

Features must import this component instead of creating MapLibre instances directly. Business behavior belongs in feature callbacks, not in `GovaMap`.

## Providers

`providers.tile` is required. All other providers are optional and injected through `GovaMapProviders`.
...
import { createAdaptiveGpsProvider } from '@/components/ui/GovaMap';

const gps = createAdaptiveGpsProvider(Geolocation);
<GovaMap providers={{ tile, gps }} modes={['tracking']} toolbar={{ gps: true }} />;
```

Request Android/iOS permissions in the native project and user flow before starting location tracking. Always stop tracking when the owning screen is dismissed; `GovaMap` also cleans up its watcher on unmount.
...
| `modes` | `GovaMapMode[]` | `['view']`. Composable behavior modes. |
| `initialViewport` | `GovaMapViewport` | Suez Governorate, Egypt at zoom 9.5 if omitted. A supplied/saved viewport always takes precedence. |
| `minZoom`, `maxZoom` | `number` | Map zoom constraints. |
| `providers` | `GovaMapProviders` | Required provider registry. |
...
| `theme` | `GovaMapTheme` | Auto theme and optional color/class overrides. |
| `markers` | `GovaMapMarker[]` | Controlled marker GeoJSON features. |
| `polygons` | `GovaMapPolygon[]` | Controlled polygon features. |
| `circles` | `GovaMapCircle[]` | Controlled geodesic-style circles expressed as centre/radius. |
| `routes` | `GovaMapRoute[]` | Controlled LineString route features. |
...
| `onMapLoaded` | Map after style and GoVa layers load. |
...
| `onError` | `GovaMapError` from initialization, providers, GPS, routes, invalid data, or an unknown source. |
...
```tsx
const [markers, setMarkers] = useState<GovaMapMarker[]>([]);
const [polygons, setPolygons] = useState<GovaMapPolygon[]>([]);
...
Attach a `ref<GovaMapHandle>` when a UI integration needs engine-level navigation:
...
`light`, `dark`, and `auto` themes are supported. Provider style changes call `map.setStyle`; the MapLibre instance is retained, and GoVa sources/layers are restored after the new style loads. Theme colors may be overridden through `GovaMapTheme`. Controlled data updates call `GeoJSONSource.setData` and do not recreate the map or React component tree.
...
Implementation: `src/components/ui/GovaMap/`. Public exports are available from both `@/components/ui/GovaMap` and `@/components/ui`.
