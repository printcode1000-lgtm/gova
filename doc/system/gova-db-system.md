# GovaDB (IndexedDB) System

Gova uses IndexedDB (GovaDB) as its primary client-side persistent storage mechanism. LocalStorage is strictly forbidden to ensure consistent behavior across platforms (especially Capacitor/mobile webviews where LocalStorage can be cleared unpredictably by the OS).
...
| `gova-dev-splash-nav-toggle` | `boolean` | Developer toggle to enable or disable automatic splash screen navigation to home. |
...
`src/lib/gova-db/index.ts` exposes the following asynchronous helper functions:
...
Because IndexedDB queries are asynchronous, Gova coordinates hydration states on mount:
...
1. **Never use LocalStorage.** Use `govaDbGet` / `govaDbSet` with the `appSettings` store instead.
2. **Handle Promises.** All GovaDB actions return a Promise and must be awaited or chained with `.then()`.
