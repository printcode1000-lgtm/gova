...
The codebase is organized into **42 sealed packages** under `packages/`, **48 application features** under `src/features/`, **6 standalone microservices** under `services/`, **7 deployment accounts** declared in `@asol/account-declarations`, and **21 databases/shards** (4 standalone/legacy databases + 8 profile shards + 9 marketplace order shards).
...
| [packages.md](./packages.md) | Comprehensive inventory of all 42 packages across 5 layers (Capability, Composition, Declarations, Bridge, Enforcement) with doors, owners, and responsibilities. |
...
| [data-flows.md](./data-flows.md) | Multi-database sharding topology (Turso/SQLite), query builder ORM usage, inode-aware SQLite caching (`CachedSqliteConnection`), Cloudflare R2 object storage hierarchy, and client cache layers. |
...
| [risks.md](./risks.md) | High-risk operational areas: static bundle overwrites, multi-account Turso schema provisioning, native Capacitor plugin sync, secret archive management, and stale deploy checkpoints. |
...
3. **Mandatory Gateways**: Database (`@asol/data-core`), Object Storage (`@asol/storage-core`), Native (`@asol/native-core`), Page Writes (`@asol/page-save-core`), Push Notifications (`@asol/notifications-core`), OTA (`@asol/ota-core`).
...
6. **Plain Source Ids**: elements are identified by the plain HTML `id` written in the source. Nothing mints, generates, validates or catalogues it; a standalone DOM inspector reads it straight off the node.
...
8. **Local Gates Gate Releases**: Releases are guarded by local verification scripts (`npm run architecture:check`, `npm run docs:ci`, `npm run runtime:check`) rather than CI workflows.
