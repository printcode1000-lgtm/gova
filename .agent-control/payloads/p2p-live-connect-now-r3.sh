...
if ! node -e "require.resolve('@esbuild/linux-x64/package.json')" >/dev/null 2>&1; then
  ...
  package_dir="$PWD/node_modules/@esbuild/linux-x64"
  mkdir -p "$package_dir"
  curl -fsSL "https://registry.npmjs.org/@esbuild/linux-x64/-/linux-x64-${ver}.tgz" | tar -xz --strip-components=1 -C "$package_dir"
fi
if ! node -e "require.resolve('werift')" >/dev/null 2>&1; then
  npm install --no-save --package-lock=false --ignore-scripts werift@0.24.4
fi
npx tsx -e 'console.log("direct-runtime-dependencies-ok")'
