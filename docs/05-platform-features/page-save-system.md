...
### Orders clear-all invariant

The Super Admin `/orders` clear-all control follows the same Page Save rule as every other user-triggered delete: the page stages the operation, the header save dialog executes it, and discarding the pending item performs no deletion. Persistence is owned by `@asol/data-core/marketplace-orders`; UI code must not issue table-level deletes or duplicate the child-first deletion sequence.
