...
5. `AsolVerificationSmsWorker` claims the dispatch id, then redeems the ticket at `/api/verification/admin-sms/redeem`. The server generates the code at redemption time, stores its digest, and returns the destination number, the final SMS body, the request id, and a signed send authorization.
