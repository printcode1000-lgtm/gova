...
After adding a separate database for the profile (`profile.db` → Turso `profile`), the build now runs **two synchronizations**:

| SQLite | Turso | Required Variables |
|--------|-------|-------------------|
| `allusers.db` | `allusers` | `TURSO_DATABASE_URL`, `TURSO_AUTH_TOKEN` |
| `profile.db` | `profile` | `TURSO_PROFILE_DATABASE_URL`, `TURSO_PROFILE_AUTH_TOKEN` |
...
```env
TURSO_PROFILE_DATABASE_URL=libsql://profile-....turso.io
