<!-- GENERATED FILE. DO NOT EDIT BY HAND.
     Source: live repository graph built by scripts/docs/.
     Regenerate: npm run architecture:docs
     Validate: npm run architecture:check -->

# Change Impact Index

Owner-level dependency edges are aggregated from live source imports. Use `npx tsx scripts/docs/context.ts <target>` for task-specific expansion.

| Subject | Kind | Docs | Tests | Routes | Runtime surfaces | Imports in | Imports out |
|---|---|---:|---:|---:|---:|---:|---:|
| `account-bridge` | feature | 1 | 0 | 0 | 0 | 0 | 2 |
| `advertisements` | feature | 2 | 2 | 6 | 0 | 3 | 14 |
| `app-reset` | feature | 1 | 0 | 0 | 0 | 2 | 2 |
| `auth` | feature | 3 | 4 | 14 | 0 | 20 | 16 |
| `cart` | feature | 1 | 4 | 2 | 0 | 5 | 11 |
| `catalog-studio` | feature | 2 | 1 | 3 | 0 | 0 | 7 |
| `categories` | feature | 4 | 0 | 8 | 0 | 13 | 6 |
| `contact` | feature | 1 | 0 | 2 | 0 | 0 | 8 |
| `data` | feature | 1 | 0 | 0 | 0 | 6 | 3 |
| `data-health` | feature | 1 | 1 | 14 | 0 | 0 | 11 |
| `dev-cloud-backup` | feature | 2 | 1 | 9 | 0 | 0 | 9 |
| `dev-tools` | feature | 1 | 0 | 1 | 0 | 0 | 8 |
| `favorites` | feature | 2 | 2 | 1 | 0 | 3 | 7 |
| `feature-flags` | feature | 1 | 1 | 1 | 0 | 0 | 5 |
| `follow` | feature | 2 | 1 | 3 | 0 | 1 | 9 |
| `google-play-console` | feature | 2 | 1 | 17 | 0 | 1 | 13 |
| `home` | feature | 2 | 0 | 1 | 0 | 0 | 4 |
| `location` | feature | 1 | 0 | 0 | 0 | 2 | 0 |
| `navigation` | feature | 1 | 0 | 0 | 0 | 0 | 4 |
| `network` | feature | 1 | 0 | 0 | 0 | 0 | 3 |
| `notifications` | feature | 9 | 29 | 14 | 0 | 9 | 10 |
| `onboarding` | feature | 1 | 0 | 0 | 0 | 1 | 10 |
| `orders` | feature | 1 | 1 | 7 | 0 | 0 | 13 |
| `ota` | feature | 8 | 0 | 3 | 0 | 0 | 5 |
| `page-save` | feature | 3 | 2 | 0 | 0 | 12 | 7 |
| `page-snapshot` | feature | 1 | 0 | 0 | 0 | 2 | 5 |
| `password-recovery` | feature | 2 | 1 | 4 | 0 | 0 | 9 |
| `pharmacy-profile-catalog` | feature | 3 | 0 | 2 | 0 | 4 | 13 |
| `product` | feature | 1 | 1 | 7 | 0 | 12 | 22 |
| `product-card` | feature | 1 | 0 | 1 | 0 | 5 | 6 |
| `product-search` | feature | 2 | 0 | 4 | 0 | 5 | 13 |
| `profile` | feature | 1 | 3 | 16 | 0 | 11 | 32 |
| `profile-products` | feature | 1 | 0 | 0 | 0 | 1 | 11 |
| `profile-working-hours` | feature | 2 | 0 | 0 | 0 | 1 | 3 |
| `qr-code` | feature | 1 | 1 | 0 | 0 | 1 | 2 |
| `release-commands` | feature | 3 | 2 | 7 | 0 | 1 | 8 |
| `seller-card` | feature | 1 | 0 | 1 | 0 | 4 | 6 |
| `seller-discounts` | feature | 1 | 1 | 3 | 0 | 2 | 7 |
| `settings` | feature | 2 | 2 | 2 | 0 | 0 | 13 |
| `sharing` | feature | 1 | 1 | 2 | 0 | 2 | 11 |
| `specialty-chat` | feature | 3 | 1 | 8 | 0 | 3 | 14 |
| `splash` | feature | 1 | 0 | 1 | 0 | 0 | 6 |
| `storage` | feature | 1 | 1 | 3 | 0 | 9 | 6 |
| `super-admin` | feature | 3 | 5 | 57 | 0 | 0 | 21 |
| `system-logs` | feature | 1 | 1 | 5 | 0 | 10 | 8 |
| `vehicle-catalog` | feature | 2 | 0 | 0 | 0 | 1 | 1 |
| `voice-input` | feature | 1 | 0 | 0 | 0 | 0 | 2 |
| `@asol/account-bridge` | package | 13 | 5 | 0 | 0 | 13 | 4 |
| `@asol/account-declarations` | package | 13 | 8 | 0 | 0 | 30 | 0 |
| `@asol/architecture-core` | package | 18 | 4 | 0 | 0 | 6 | 7 |
| `@asol/auth-core` | package | 10 | 6 | 6 | 0 | 57 | 2 |
| `@asol/backup-core` | package | 7 | 2 | 0 | 0 | 11 | 3 |
| `@asol/branding-core` | package | 12 | 1 | 0 | 0 | 8 | 1 |
| `@asol/catalog-core` | package | 8 | 1 | 0 | 0 | 17 | 1 |
| `@asol/data-core` | package | 52 | 23 | 5 | 0 | 159 | 20 |
| `@asol/data-health-core` | package | 7 | 2 | 0 | 0 | 45 | 0 |
| `@asol/dev-core` | package | 8 | 1 | 0 | 0 | 10 | 0 |
| `@asol/env-core` | package | 6 | 1 | 0 | 0 | 14 | 1 |
| `@asol/featured-marquee-core` | package | 6 | 1 | 1 | 0 | 12 | 1 |
| `@asol/format-core` | package | 7 | 2 | 1 | 0 | 37 | 0 |
| `@asol/google-play-store-assets-core` | package | 7 | 2 | 6 | 0 | 16 | 0 |
| `@asol/hero-slider-core` | package | 7 | 2 | 1 | 0 | 21 | 1 |
| `@asol/map-core` | package | 10 | 1 | 0 | 0 | 9 | 7 |
| `@asol/native-core` | package | 31 | 20 | 0 | 0 | 79 | 27 |
| `@asol/notifications-composition` | package | 10 | 1 | 0 | 0 | 2 | 4 |
| `@asol/notifications-core` | package | 20 | 14 | 0 | 0 | 88 | 5 |
| `@asol/observability-core` | package | 8 | 1 | 1 | 0 | 21 | 5 |
| `@asol/orders-composition` | package | 12 | 1 | 0 | 0 | 2 | 5 |
| `@asol/orders-core` | package | 17 | 6 | 4 | 0 | 41 | 1 |
| `@asol/ota-core` | package | 34 | 26 | 3 | 0 | 40 | 13 |
| `@asol/page-save-core` | package | 26 | 3 | 0 | 0 | 8 | 0 |
| `@asol/page-snapshot-core` | package | 8 | 1 | 0 | 0 | 3 | 0 |
| `@asol/product-core` | package | 9 | 1 | 0 | 0 | 26 | 0 |
| `@asol/product-style-core` | package | 9 | 1 | 1 | 0 | 8 | 0 |
| `@asol/products-composition` | package | 8 | 1 | 0 | 0 | 4 | 6 |
| `@asol/profiles-composition` | package | 8 | 1 | 0 | 0 | 6 | 3 |
| `@asol/release-core` | package | 8 | 5 | 4 | 0 | 45 | 4 |
| `@asol/secrets-core` | package | 5 | 1 | 0 | 0 | 8 | 0 |
| `@asol/service-mirror-core` | package | 10 | 1 | 0 | 0 | 7 | 1 |
| `@asol/service-runtime-core` | package | 7 | 1 | 0 | 0 | 17 | 0 |
| `@asol/signed-token-core` | package | 7 | 1 | 0 | 0 | 8 | 0 |
| `@asol/storage-core` | package | 31 | 22 | 2 | 0 | 97 | 5 |
| `@asol/storage-image-manager-core` | package | 9 | 2 | 0 | 0 | 12 | 8 |
| `@asol/sub2main-composition` | package | 6 | 1 | 0 | 0 | 2 | 7 |
| `@asol/submain-composition` | package | 6 | 1 | 0 | 0 | 5 | 7 |
| `@asol/system-logs-core` | package | 9 | 4 | 2 | 0 | 52 | 0 |
| `@asol/trending-ribbon-core` | package | 6 | 1 | 1 | 0 | 11 | 1 |
| `@asol/vercel-deploy-core` | package | 7 | 4 | 0 | 0 | 26 | 1 |
| `notifications` | service | 6 | 0 | 0 | 0 | 0 | 3 |
| `orders` | service | 4 | 0 | 0 | 0 | 0 | 4 |
| `products` | service | 4 | 0 | 0 | 0 | 0 | 3 |
| `profiles` | service | 4 | 0 | 0 | 0 | 0 | 4 |
| `sub2main` | service | 2 | 0 | 0 | 0 | 0 | 3 |
| `submain` | service | 2 | 0 | 0 | 0 | 0 | 4 |
