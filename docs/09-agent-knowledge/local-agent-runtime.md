...
- Mode C is the Remote Desktop Commander-only path. When selected, Remote Desktop Commander is the exclusive transport for the complete task: all project/device reads, searches, edits, commands, tests, Git operations, service/process actions, builds, and separately authorized external-service/deployment commands execute through the paired device connection. Its first device command is `python3 /home/hesham/gova/tools/local-agent/mode_c_preflight.py`. Mode C does not execute through the Gateway, GitHub workflows, the Mode-B projector, a public tunnel, or direct cloud repository tools, and it never falls back to another mode implicitly.
