...
### Four bugs this check had before it worked

Worth recording, because each one made it silently permissive or falsely strict:
...
4. Line splitting used `content.split('\n')` on CRLF working copies. Each marker line
   kept a trailing `\r`, so the `page-save-read` regex never matched and valid read-shaped
   POSTs in `use-data-health-page.ts` and `use-dev-cloud-backup-page.ts` failed
   `architecture:check` on Windows while passing on LF-only CI. Fixed by splitting on
   `/\r?\n/`. See
   [page-save-read-marker-crlf-windows.md](../08-troubleshooting/problems/page-save-read-marker-crlf-windows.md).
