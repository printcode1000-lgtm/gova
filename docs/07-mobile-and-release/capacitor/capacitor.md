...
| `npm run android:preflight`         | Resolve JDK 21, Android SDK root, and the Gradle wrapper before any APK/AAB build |
...
- **Android build preflight:** every repository command that assembles an APK or AAB runs `packages/native-core/scripts/android-build-preflight.ts` **before** Gradle starts. It resolves invalid `JAVA_HOME` / `ASOL_ANDROID_JAVA_HOME` by searching Android Studio JBR and common `jdk-21*` install directories, resolves the Android SDK root from `ANDROID_SDK_ROOT` / `ANDROID_HOME` / `android/local.properties` / the default `%LOCALAPPDATA%\Android\Sdk`, and requires the checked-in wrapper under `android/`. When a valid JDK is discovered it is injected into the child Gradle process even if the user `JAVA_HOME` is wrong. On failure it exits with code `1` and prints a bilingual English/Arabic error showing the configured path, every searched JDK location, and the resolved values. Run it alone with `npm run android:preflight`.
