...
Canonical inventory of every sealed `@asol/*` package in `packages/`. Each record is verified against `packages/<folder>/package.json` `exports` and `packages/architecture-core/src/registry/capability-registry.ts`.

## Scope

Covers all 41 sealed packages under `packages/`. Does not cover `services/*/generated/` mirrors (those are deployment copies synced by `@asol/service-mirror-core`).

## Source of Truth

This document is the canonical package inventory. Capability ownership statements defer to [capability-map.md](./capability-map.md).
...
| **Runtime** | universal (single or multi-door) |
| **Public Exports** | `.` |
...
| **Runtime** | universal (single or multi-door) |
| **Public Exports** | `.` · `./gova` · `./notifications` · `./products` · `./orders` · `./profiles` · `./submain` · `./sub2main` |
| **Infrastructure Privileges** | none |
| **May Import App (`@/`)** | no |
...
| **Architectural Layer** | capability |
| **Runtime** | native + browser shell |
| **Public Exports** | `.` · `./platform-globals` · `./scripts/validate-android-r8-policy` · `./scripts/android-build-preflight` |
...
| **Architectural Layer** | capability |
| **Runtime** | universal + ./server |
| **Public Exports** | `.` · `./server` · `./builder` · `./providers` |
...
| **Architectural Layer** | capability |
| **Runtime** | universal (single or multi-door) |
| **Public Exports** | `.` · `./console` · `./console-server` · `./console-artifacts` · `./console/android-release-runbook` |
| **Infrastructure Privileges** | none |
| **May Import App (`@/`)** | no |
...
| **Architectural Layer** | capability |
| **Runtime** | universal + ./server |
| **Public Exports** | `.` · `./server` · `./profiles-config` |
...
| **Architectural Layer** | capability |
| **Runtime** | Node-only deploy engine |
| **Public Exports** | `.` |
| **Infrastructure Privileges** | none |
| **May Import App (`@/`)** | no |
...
- [Package Layers](../02-packages/package-layers.md)
...
Adding or removing a package requires updating: `capability-registry.ts`, this catalog, `capability-map.md`, `package.json` exports, root `tsconfig.json` paths, ESLint restrictions, and the matching `test:*-core` gate in root `package.json` build chain.
