...
All routes that render `AppHeader` pass through `AppShell` and share one page-start rule with no per-page exceptions. `/home` is the canonical visual reference: its page root has no top margin or top padding, and its first real child has no external top margin. The global shell uses one fixed visual gap everywhere: `--asol-shell-main-gap` is `3px` on phones, small, medium, and large screens. `.asol-shell-main > :first-child` still forces the page root's top margin and top padding to `0`, and `.asol-shell-main > :first-child > :first-child` forces the first element's top margin to `0`, so the only header-to-content spacing is the shared 3px shell gap. Horizontal padding, bottom padding, and the first component's internal padding remain page-owned.
