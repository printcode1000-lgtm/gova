...
## App header content spacing

All routes that render `AppHeader` pass through `AppShell` and share one page-start rule with no per-page exceptions. `/home` is the canonical visual reference: its page root has no top margin or top padding, and its first real child has no external top margin. The global shell uses one fixed visual gap everywhere: `--asol-shell-main-gap` is `6px` on phones, small, medium, and large screens. `.asol-shell-main > :first-child` still forces the page root's top margin and top padding to `0`, and `.asol-shell-main > :first-child > :first-child` forces the first element's top margin to `0`, so the only header-to-content spacing is the shared 6px shell gap. Horizontal padding, bottom padding, and the first component's internal padding remain page-owned.

The shell still reserves the complete header height, including the native safe-area inset and the optional install-prompt row. Pages must not add or override top spacing merely to clear the fixed header; that clearance is owned centrally by `.asol-shell-main`, and the architecture gate rejects alternate shell-gap declarations or shell-specific CSS outside `globals.css`. Intentional spacing belongs after the first page element. The `/` splash route has no `AppHeader`, so there is no header-to-page spacing relationship to apply there.
