...
- `npm run ui-registry:apply-pending` — apply what can be proven, re-run the UiRegistry contract over the result, and exit non-zero if anything remains open. It reads the same store the app wrote to, so applying requests raised in production needs the production credentials in the environment.
...
**Open dialogs stay open.** The inspector, the floating error toolbar, and the development badge are overlay chrome (`data-asol-overlay-chrome`). `DialogContent` and `DropdownMenuContent` ignore pointer/focus dismiss from that chrome, so enabling the inspector or tapping it to select an element on `PageSaveDialog` (or any other shared dialog) does not close the dialog. The expanded error toolbar and the app sidebar also ignore taps on that chrome **and** on another open `role="dialog"` layer, so inspecting a save dialog does not close the sidebar. The sidebar focus trap allows those same outside taps. Tapping the dimmed overlay or the dialog's own close control still dismisses it. `npm run test:overlay-chrome` pins the helper, the shared dismiss wiring, and the chrome markers.
