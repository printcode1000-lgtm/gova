...
/**
 * Build config for the orders service.
 *
 * Deliberately minimal: this app serves read-only order APIs and no pages, so
 * its build touches no database at all. That is what lets the products Vercel
 * account hold the order shards credentials and nothing else.
 */
