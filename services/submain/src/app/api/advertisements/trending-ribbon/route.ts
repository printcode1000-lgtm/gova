...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    if (url.searchParams.get('admin') !== '1') {
      const current = await advertisements.trendingRibbon.getCurrent();
      return jsonResponse(request, current, 200);
    }
    const admin = await advertisements.trendingRibbon.getAdmin({
...
export async function PUT(request: Request): Promise<Response> {
  try {
    ...
    const saved = await advertisements.trendingRibbon.save(
      body.identity,
      body.config,
    ...
    return advertisementsSaveErrorResponse(request, error, 'invalidTrendingRibbonConfig');
  }
}
