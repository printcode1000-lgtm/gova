...
export async function GET(_request: Request, context: { params: Promise<{ revision: string }> }) {
  ...
  if (!SHA.test(revision)) return jsonContractResponse({ error: 'invalid_revision' }, { status: 400 });
  try {
    return jsonContractResponse({ revision, status: await releaseReadinessFor(revision) });
  } catch (error) {
    ...
    return jsonContractResponse({ revision, status: 'pending' as const });
  }
}
