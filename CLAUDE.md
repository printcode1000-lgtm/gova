...
11. **Never leave `public/sync_data` or the secrets backup behind (mandatory).** Any change under `public/sync_data`, and any file `npm run secrets:backup` writes into `config/`, must be committed and pushed to GitHub in the same change that produced it. `.githooks/pre-push.d/20-sync-data-committed` and `30-secrets-backup-committed` refuse the push while either is dirty. Do not bypass with `--no-verify`, and do not "solve" it by adding these paths to `.gitignore` — they are tracked on purpose (schema sources, and a backup that must be restorable from a fresh clone).
